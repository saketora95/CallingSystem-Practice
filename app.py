...
def get_login_page():
    if request.method == 'POST':
        ...
        if login.counter_login(login_data['user_id'], login_data['pwd']):
            ...
            return render_template('login_page.html', msg='登入失敗！您所輸入的帳號或是密碼有誤，請重新確認後再嘗試。')
...
def get_counter_select_page():
    return render_template('counter_select_page.html')

# 顯示對應 `counter_id` 目前的叫號號碼。
@app.route('/counter/<counter_id>')
def get_counter_display_page():
    return render_template('counter_display_page.html')
